...
% K_init_list = [1, 10, 100, 1000, 10000];
% K_0_init_list = [1, 10, 100, 1000, 10000];
minibatch_sz = 200;
batch_num_list = 1:20;
...
X_0_init_list = [0.9999:-0.005:0.0999, 0.0899:-0.0005:0.0099, 0.0089:-0.0001:0.0009, 0.0008:-0.00001:0.0001, 0.00009:-0.000001:0.00001];
...
for i1 = 1 : length(K_init_list)
    ...
    for i2 = 1 : length(X_0_init_list)
        ...
        for i3 = 1 : length(batch_num_list)
            ...
            cons4 = isempty(   find( (a_1*c_1/((1-X_0)*N*K) + a_2*c_2*(1-X_0^3)*K^2 / (1-X_0) ...
                + a_3*(1-X_0^2)/(1-X_0)*(c_3/B+c_4*sum(q_s0sn)*K/N) <= C_max)==0, 1)   );
...
while norm([K_t; K_0_t; B_t]-[K; K_0; B], 2)>0.1
    ...
    lambda_0 = (C_max+a_2*c_2*K^2*X_0_t^3+a_3*c_3*X_0_t^2/B_t)*K*N + a_3*c_4*X_0_t^2*sum(q_s0sn)*K^2;
    lambda_1 = C_max*K/lambda_0;
    lambda_2 = a_2*c_2*K^3*X_0_t^3/lambda_0;
    ...
    ( a_1*c_1 + (a_2*c_2*K^2+a_3*c_3/B+C_max*X_0)*K*N + a_3*c_4*sum(q_s0sn)*K^2 ) ...
        / prod( ((C_max*K/lambda_1)^lambda_1) .* ((a_2*c_2*K^3*X_0^3/lambda_2)^lambda_2) ...
