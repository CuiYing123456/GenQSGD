...
str=[' -';' :';'--'; '-.'];
mkr = ['o';'+';'s';'^';'*';'d'];
pathname_tmp = 'F:\Research\MyPaper\GenQSGD_Journal\Simulation\s_Hetero_dim\Tmp\';
pathname_data = 'F:\Research\MyPaper\GenQSGD_Journal\Simulation\s_Hetero_dim\Data\';
rand_stop = 20;
...
for j = 1 : length(gamma_dim_list)
    ...
    % ------------ Plot 1 ---------
    p1 = plot(s_ratio_list, energy_line_dim);
...
line2=semilogy(s_ratio_list, energy_line_dim,'r--o',s_ratio_list, energy_line_PM_fix,'b--+',s_ratio_list, energy_line_FedAvg_fix,'g--s',s_ratio_list, energy_line_PR_fix,'m--^');
...
ax.YAxis.Exponent = 3;
ax.Box = 'off';
axis([min(s_ratio_list), max(s_ratio_list), 0.99*energy_min, 1.2*energy_max]);
