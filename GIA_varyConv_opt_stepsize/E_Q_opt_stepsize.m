function [ energy, K_out, B_out, K_0_out, gamma] = E_Q_opt_stepsize( N, s_n, s_0, D, alpha, ...
...
for i = 1 : 2 : 500
    if floor( K_0_tmp ) + i - 300 > 0
        K_0 = floor(K_0_tmp) + i - 300;
    end
    for j1 = 1 : 2 : 20
        if floor( K_tmp(1) ) + j1 - 15 >= 0
            K(1:floor(N/2)) = floor( K_tmp(1:floor(N/2)) ) + j1 - 15;
        end
        for j2 = 1 : 2 : 20
            if floor( K_tmp(end) ) + j2 - 15 >= 0
                K(floor(N/2)+1:end) = floor( K_tmp(floor(N/2)+1:end) ) + j2 - 15;
            end
            for k = 1 : 2 : 20
                ...
                for n = 1 : 20
                    ...
                    cons4 = isempty(   find( (c_1/(gamma*K_0*sum(K)) + c_2*gamma^2*T_2^2 ...
                        + c_3*gamma/B + c_4*gamma*sum(q_s0sn.*K.^2)/sum(K) <= C_max)==0, 1)   );
                    if cons3 && cons4 && energy_tmp-energy_min < rcv_error
                        energy = energy_tmp;
                        rcv_error = energy_tmp-energy_min;
