function [ energy, K_out, B_out, K_0_out ] = E_Q_exp( N, s_n, s_0, D, alpha, ...
...
% d_n = 2*L*gamma*G^2*(q_0*q_n+q_0+q_n);
...
% K_init_list = [ 1, 6:12, 18:2:36, 80:10:120, 180:20:360, 800:100:1200];
% % K_0_init_list = [ 1, 50, 100, 200, 1000, 10000, 100000, 1000000];
% B_init_list = [ 1:10, 20:10:100, 200:100:1000, 2000:1000:10000, 20000:10000:100000];
% X_0_init_list = [0.99999, 0.99995, 0.9999, 0.9995, 0.999, 0.995, 0.99, 0.95, 0.9, 0.5, 0.09, 0.05, 0.009, 0.005, 0.0009, 0.0005, 0.00009];

K_init_list = [ 1:20, 30:10:120, 140:20:800, 900:100:2000];
B_init_list = [ 1:20, 30:10:100, 200:100:1000, 2000:1000:10000];
...
while norm([K_t; K_0_t; B_t]-[K; K_0; B], 2)>1
    K_t = K;
    K_0_t = K_0;
    B_t = B;
    T_2_t = T_2;
...
% 
% K(1:floor(N/2)) = floor( K(1:floor(N/2)) );
% K(floor(N/2)+1:end) = floor( K(floor(N/2)+1:end) );
% B = floor(B);
% K_0 = floor(K_0);
    ...
    for j1 = 1 : 2 : 10
        ...
        for j2 = 1 : 2 : 10
            ...
            for k = 1 : 2 : 10
                ...
                T_1 = max(C_n./F_n.*K);	T_2 = max(K);   X_0 = rho_E^K_0;
                cons3 = isempty(   find( ((B*T_1+C_0/F_0+max(M_n./r_n)+M_0/r_0)*K_0 <= T_max)==0, 1 )   );
                cons4 = isempty(   find( (a_1*c_1/((1-X_0)*sum(K)) + a_2*c_2*(1-X_0^3)*T_2^2 / (1-X_0) ...
                    + a_3*(1-X_0^2)/(1-X_0)*(c_3/B+c_4*sum(q_s0sn.*K.^2)/sum(K)) <= C_max)==0, 1)   );
