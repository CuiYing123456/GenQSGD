...
K_init_list = [ 1:20, 50:5:120, 130:10:800, 900:100:2000];
...
while norm([K_t; K_0_t]-[K; K_0], 2)>0.1
    ...
    lambda_0 = (C_max+a_2*c_2*T_2_t^2*X_0_t^3+a_3*c_3*X_0_t^2/B)*sum(K_t) + a_3*c_4*X_0_t^2*sum(q_s0sn.*K_t.^2);
    lambda_1 = C_max*K_t/lambda_0;
    lambda_2 = a_2*c_2*T_2_t^2*X_0_t^3*K_t/lambda_0;
    lambda_3 = a_3*c_3*X_0_t^2*K_t/(B*lambda_0);
    ...
    cvx_precision low
    cvx_solver SeDuMi
    variables K(N,1) K_0 T_1 T_2 X_0
...
K_0_tmp = K_0;  K_tmp = K;
for i = 1 : 20: 4000
    if floor( K_0_tmp ) + i - 2000 > 0
        K_0 = floor(K_0_tmp) + i - 2000;
    end
    for j1 = 1 : 1: 20
