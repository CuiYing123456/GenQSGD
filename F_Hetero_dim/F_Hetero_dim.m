...
str=[' -';' :';'--'; '-.'];
mkr = ['o';'+';'s';'^';'*';'d'];
pathname_tmp = 'F:\Research\MyPaper\GenQSGD_Journal\Simulation\F_Hetero_dim\Tmp\';
pathname_data = 'F:\Research\MyPaper\GenQSGD_Journal\Simulation\F_Hetero_dim\Data\';
rand_stop = 20;
...
gamma_dim_list = [0.02];
rho_D = 600;
for j = 1 : length(gamma_dim_list)
    gamma = gamma_dim_list(j);
    % --------- Simulation 1 ---------------
    energy_line_dim = [];
    ...
    for i = 1 : length(F_ratio_list)
        ...
        %     F_mean = F_max*(1+F_ratio)/(2*F_ratio);
        %     F_min = F_mean * 2 / (1 + F_ratio);
    ...
    save(strcat(pathname_data, 'Energy_dim_gamma=', num2str(gamma_dim_list(j)), '.mat'), 'energy_line_dim');
    
    % ------------ Plot 1 ---------
    p1 = plot(F_ratio_list, energy_line_dim);
    ...
    energy_max = max([ energy_line_dim(energy_line_dim~=Inf) ]);
    energy_min = min([energy_line_dim]);
end
...
% Lgd_cnt = 1;
% for j = 1 : length(gamma_dim_list)
%     load(strcat(pathname_data, 'Energy_dim_gamma=', num2str(gamma_dim_list(j)), '.mat'));
%     energy_min = min(energy_line_dim);
%     energy_max = max(energy_line_dim);
%     % ------------ Plot 1 ---------
%     p1 = semilogy(F_ratio_list, energy_line_dim);
...
line2=semilogy(F_ratio_list, energy_line_dim,'r--o',F_ratio_list, energy_line_PM_fix,'b--+',F_ratio_list, energy_line_FedAvg_fix,'g--s',F_ratio_list, energy_line_PR_fix,'m--^');
