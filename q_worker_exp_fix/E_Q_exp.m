...
%% =============== Solution: Proposed =================
% ------------- Initialize --------------
% K = 300*ones(N,1);   K_t = zeros(N, 1);
% B = 1;     B_t = 0;
% T_1 = max(C_n./F_n.*K);   T_1_t = 0;
% T_2 = max(K);
% X_0 = 0.00001;
% K_0 = log(X_0)/log(rho_E);   K_0_t = 0;
...
% B_init_list = [ 1:10, 20:10:100, 200:100:1000, 2000:1000:10000];
    ...
    for j1 = 1 : 2 : 20
        ...
        for j2 = 1 : 2 : 20
            ...
            for k = 1 : 2 : 20
                ...
                for n = 1 : 2 : 40
                    if gamma_tmp - 0.04 + 0.04*n/20 >= 0
                        gamma = gamma_tmp - 0.04 + 0.04*n/20;
                    end
                    energy_tmp = K_0 * (B*sum(alpha*C_n.*F_n.^2.*K) + alpha*C_0*F_0^2 + sum(p_n.*M_n./r_n) + p_0*M_0/r_0);
