...
Acc_rcd_opt = zeros(length(B_list), 1);  Loss_rcd_opt = zeros(length(B_list), 1);

for i = 1 : length(B_list)
    B = B_list(i);  K_1 = K_1_list(i);  K_2 = K_2_list(i);  K_0 = K_0_list(i);  lr = lr_list(i);
    J_step = zeros(L, 1);  accur = zeros(L, 1);
    ...
    for iter=1:L
        ...
        J_step(iter) = nnCostFunction(Theta1_global, Theta2_global, num_labels, img_train, lab_train, lambda);
        pred = Predict(Theta1_global, Theta2_global, img_test);
        accur(iter)=mean(double(pred == lab_test)) * 100;
        fprintf('\nTest accuracy of iteration %d (opt): %f', iter,...
            mean(double(pred == lab_test)) * 100);
        
        Acc_rcd_opt(i) = sum(accur)/iter;
        Loss_rcd_opt(i) = sum(J_step)/iter;
        save(sprintf('./Data/Acc_rcd_opt.mat'), 'Acc_rcd_opt');
        save(sprintf('./Data/Loss_rcd_opt.mat'), 'Loss_rcd_opt');
    end 
end
...
Acc_rcd_cons = zeros(length(B_list), 1);  Loss_rcd_cons = zeros(length(B_list), 1);
...
for i = 1 : length(B_list)
    ...
    for iter=1:L
        ...
        pred = Predict(Theta1_global, Theta2_global, img_test);
        accur(iter)=mean(double(pred == lab_test)) * 100;
        fprintf('\nTest accuracy of iteration %d (cons,C_max=%0.2f): %f', iter, G_th_list(i), ...
            mean(double(pred == lab_test)) * 100);
        
        Acc_rcd_cons(i) = sum(accur)/iter;
        Loss_rcd_cons(i) = sum(J_step)/iter;
        save(sprintf('./Data/Acc_rcd_cons.mat'), 'Acc_rcd_cons');
        save(sprintf('./Data/Loss_rcd_cons.mat'), 'Loss_rcd_cons');
    end 
end
...
rho_E = 0.9995;
% B_list = [1,1,1,1,1,1];
% K_1_list = [2,2,1,0,0,0];
% K_2_list = [2,2,2,2,2,2];
% K_0_list = [1371,1265,1480,1439,1400,1365]; 
B_list = [3,3,3,3,3,3];
K_1_list = [2,2,2,2,2,2];
K_2_list = [2,2,2,2,2,2];
K_0_list = [1428,1306,1200,1111,1032,963]; 
Acc_rcd_exp = zeros(length(B_list), 1);  Loss_rcd_exp = zeros(length(B_list), 1);
...
for i = 1 : length(B_list)
    ...
    for iter=1:L
        ...
        pred = Predict(Theta1_global, Theta2_global, img_test);
        accur(iter)=mean(double(pred == lab_test)) * 100;
        fprintf('\nTest accuracy of iteration %d (exp): %f', iter,...
            mean(double(pred == lab_test)) * 100);
        
        Acc_rcd_exp(i) = sum(accur)/iter;
        Loss_rcd_exp(i) = sum(J_step)/iter;
        save(sprintf('./Data/Acc_rcd_exp.mat'), 'Acc_rcd_exp');
        save(sprintf('./Data/Loss_rcd_exp.mat'), 'Loss_rcd_exp');
    end
end
...
rho_D = 600;
% B_list = [2,2,2,2,2,2];
% K_1_list = [2,2,2,2,2,1];
% K_2_list = [2,2,2,2,2,3];
% K_0_list = [1765,1631,1516,1415,1326,1514]; 
B_list = [3,3,3,3,3,3];
K_1_list = [3,3,3,3,3,3];
K_2_list = [3,3,3,3,3,3];
K_0_list = [1631,1495,1379,1280,1192,1115]; 
Acc_rcd_dim = zeros(length(B_list), 1);  Loss_rcd_dim = zeros(length(B_list), 1);
...
for i = 1 : length(B_list)
    ...
    for iter=1:L
        ...
        pred = Predict(Theta1_global, Theta2_global, img_test);
        accur(iter)=mean(double(pred == lab_test)) * 100;
        fprintf('\nTest accuracy of iteration %d (dim): %f', iter,...
            mean(double(pred == lab_test)) * 100);
        
        Acc_rcd_dim(i) = sum(accur)/iter;
        Loss_rcd_dim(i) = sum(J_step)/iter;
        save(sprintf('./Data/Acc_rcd_dim.mat'), 'Acc_rcd_dim');
        save(sprintf('./Data/Loss_rcd_dim.mat'), 'Loss_rcd_dim');
    end
