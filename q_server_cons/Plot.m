...
line2=loglog(bits_list(2:end), energy_line_cons(2:end),'r--o',bits_list(2:end), energy_line_PM_fix(2:end),'b--+',bits_list(2:end), energy_line_FedAvg_fix(2:end),'g--s',bits_list(2:end), energy_line_PR_fix(2:end),'m--^');
...
ax.YAxis.Exponent = 4;
ax.Box = 'off';
axis([min(bits_list(2:end)), max(bits_list), 0.95*energy_min, 1.5*energy_max]);
